...
2. **No próprio dashboard**: `acordar_api_em_segundo_plano()` (em `src/dashboard/app.py`) dispara um `/health` numa thread solta assim que a landing abre, antes do login. A API sobe em paralelo com a leitura da página e a digitação da senha, em vez de só começar a acordar depois.
3. **Fora da aplicação**: um pinger externo (cron-job.org, gratuito e sem cartão) chama as duas URLs a cada 10 min, seg-sex das 9h às 19h, no fuso de São Paulo. É o que evita o cold start no horário em que o sistema é usado de verdade.

**Por que o keepalive não é um workflow do GitHub Actions:** existiu um `keepalive.yml` e foi removido. Agendamento do GitHub Actions em repositório gratuito não é honrado: medindo o histórico real de execuções, um `*/10` rodava a cada 2-4 HORAS e chegou a ficar 14h sem rodar. Pior, as execuções que aconteciam caíam de madrugada (00h, 05h), acordando os serviços quando ninguém usa e gastando horas do limite à toa. O `cron.yml` (coleta noturna) continua no GitHub Actions porque ali o atraso não importa — e ele acorda a API com `/health` antes de disparar o job.

**Intervalo de 10 minutos, não 15:** o Render desliga o serviço com 15 min de inatividade. Um ping a cada 15 min empata com esse limiar e deixa o serviço dormir parte das vezes.
